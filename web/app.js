...
const helpSlides = [
    ...
    },
    {
        ...
        html: () => `
            ...
            <p>Docker is the intended runtime. The core container runs non‑root and without privileged mode, Docker‑socket access or device mounts. All project code and build commands execute behind a controlled boundary with central approval-based capability registration bound to executable identity, denied‑argument patterns, timeouts and output limits. Future toolchains follow Project Intelligence → Engineering Council → Chairman approval → Human Approval → controlled capability registration → controlled execution; approval never authorizes arbitrary shell commands, images, argv, mounts or devices. Toolchains are not preinstalled — verification reports tool_unavailable when a required tool is missing.</p>
