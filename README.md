...
**Generalized Verification** leitet aus der Project Intelligence automatisch einen strukturierten, area-spezifischen VerificationPlan ab. Pro Project Area wird anhand des erkannten Test- oder Build-Systems ein kontrollierter Runner gewählt: pytest und Python unittest werden direkt ausgeführt; vitest, jest, cmake, make, platformio und esphome werden als deferred/unsupported markiert und nicht blind gestartet. Kein Runner führt `shell=True`-Kommandos, `npm run <script>` oder beliebige LLM-Befehle aus. Unbekannte Systeme sind `unsupported`, nicht `PASS`. Mixed Projects erhalten mehrere Verification Steps mit jeweils eigener Area, eigenem Working Directory und eigenem Runner. Die Aggregation aller Steps bestimmt das PASS-/FAIL-Gesamtergebnis — ein unsupported Pflichtschritt erzeugt kein PASS.
